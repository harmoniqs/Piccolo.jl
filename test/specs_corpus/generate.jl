...
using Piccolo: Specs
using Piccolo.Specs:
    ProblemSpec, SystemSpec, GoalSpec, PulseSpec, TemplateBlock,
    IntegratorSpec, WrapperSpec, ObjectiveTermSpec, Free, Fixed
using TOML

const TRANSMON_PARAMS = Dict{Symbol,Any}(
    :n_qubits => 1, :levels => 3, :ω => 4.0e0, :δ => -2.0e-1,
)
...
function corpus_specs()
    ...
    transmon(args...; kwargs...) = SystemSpec(;
        kind = :template, template = :TransmonSystem,
        params = TRANSMON_PARAMS, args..., kwargs...
    )
    push!(specs, "smooth-unitary-zo" => ProblemSpec(;
        system = transmon(),
    ...
    push!(specs, "smooth-ket-zo" => ProblemSpec(;
        system = transmon(),
    ...
    push!(specs, "spline-unitary-linear-freephase" => ProblemSpec(;
        system = transmon(),
        goal = GoalSpec(kind = :unitary, gate = :X),
        pulse = PulseSpec(kind = :linear_spline, T = 30.0, init = :default, seed = 0),
        problem = TemplateBlock(;
            template = :SplinePulseProblem, N = 200, free_phase = true,
            R_du = 1.0e-2, du_bound = 0.3,
        ),
        integrator = IntegratorSpec(kind = :spline, alg = :tsit5),
    ))
    push!(specs, "spline-unitary-cubic-freephase" => ProblemSpec(;
        system = transmon(),
    ...
    push!(specs, "spline-ket-cubic" => ProblemSpec(;
        system = transmon(),
        goal = GoalSpec(kind = :ket, target = "(g0+e0)/√2"),
        pulse = PulseSpec(kind = :cubic_spline, T = 15.0, init = :default, seed = 0),
        problem = TemplateBlock(template = :SplinePulseProblem, N = 75, free_phase = true),
        integrator = IntegratorSpec(kind = :spline, alg = :tsit5),
    ))
    push!(specs, "bangbang-unitary-zo" => ProblemSpec(;
        system = transmon(),
        goal = GoalSpec(kind = :unitary, gate = :X),
    ...
    push!(specs, "bangbang-ket-zo-freephase" => ProblemSpec(;
        system = transmon(),
        goal = GoalSpec(kind = :ket, target = "e0"),
        pulse = PulseSpec(kind = :zero_order, T = 12.0, init = :random, seed = 3),
        problem = TemplateBlock(;
            template = :BangBangPulseProblem, N = 60, free_phase = true,
            initial_phases = [0.0, 0.0],
        ),
    ))
    push!(specs, "smooth-mintime-recipe" => ProblemSpec(;
        system = transmon(),
        goal = GoalSpec(kind = :unitary, gate = :X),
        pulse = PulseSpec(kind = :zero_order, T = 20.0, init = :zeros, seed = 0),
        problem = TemplateBlock(;
            template = :SmoothPulseProblem, N = 100,
            goal_treatment = :both, final_fidelity = 0.999,
            free_dt = Free(5.0, 25.0),
            objectives = [ObjectiveTermSpec(kind = :time, weight = 1.0)],
        ),
    ))
    push!(specs, "sampling-wrapped" => ProblemSpec(;
        system = SystemSpec(;
            kind = :template, template = :TransmonSystem,
    ...
    push!(specs, "transmon-globals-unpinned" => ProblemSpec(;
        system = SystemSpec(;
            kind = :template, template = :TransmonSystem,
            params = TRANSMON_PARAMS,
    ...
    matrix = Any[
        Any[Any[1.0, 0.0], Any[0.0, 0.0]],
        Any[Any[0.0, 0.0], Any[0.0, 1.0]],
    ]
    push!(specs, "smooth-unitary-inline-matrix" => ProblemSpec(;
        system = transmon(),
