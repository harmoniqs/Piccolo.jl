"""
    KnotPointPropagationData

Cached forward-rollout data for efficient VJP / JVP computation.

Stores propagators, eigendecompositions, intermediate states, and system
information produced by [`setup_knot_point_propagation`](@ref).
"""
struct KnotPointPropagationData
    propagators::Vector{Matrix{ComplexF64}}
    eigenvalues::Vector{Vector{Float64}}
    eigenvectors::Vector{Matrix{ComplexF64}}
    states::Vector{Vector{ComplexF64}}
    H_drift::Matrix{ComplexF64}
    H_drives::Vector{Matrix{ComplexF64}}
    controls::Matrix{Float64}
    Δts::Vector{Float64}
end

"""
    setup_knot_point_propagation(sys, controls, Δts, ψ0)

Perform a forward rollout of the knot-point dynamics, computing and caching
all propagators via eigendecomposition at each interval.

# Arguments
- `sys`: a `QuantumSystem` (provides `H_drift`, `H_drives`, `n_drives`, `levels`)
- `controls::AbstractMatrix`: size `[n_drives × N]`, control amplitudes at each knot point
- `Δts::AbstractVector`: time-step durations (length ≥ N-1)
- `ψ0::AbstractVector`: initial ket state

Returns a [`KnotPointPropagationData`](@ref).
"""
function setup_knot_point_propagation(
    sys,
    controls::AbstractMatrix,
    Δts::AbstractVector,
    ψ0::AbstractVector,
)
    n_drives = sys.n_drives
    N = size(controls, 2)
    n_intervals = N - 1

    H_drift = Matrix{ComplexF64}(sys.H_drift)
    H_drives = [Matrix{ComplexF64}(d.H) for d in sys.H_drives]

    propagators = Vector{Matrix{ComplexF64}}(undef, n_intervals)
    eigenvalues = Vector{Vector{Float64}}(undef, n_intervals)
    eigenvectors = Vector{Matrix{ComplexF64}}(undef, n_intervals)
    states = Vector{Vector{ComplexF64}}(undef, N)

    states[1] = ComplexF64.(ψ0)

    for j = 1:n_intervals
        # Total Hamiltonian at knot point j
        H_total = copy(H_drift)
        for k = 1:n_drives
            H_total .+= controls[k, j] .* H_drives[k]
        end

        # Eigendecomposition
        λ, V = eigen(Hermitian(H_total))
        eigenvalues[j] = λ
        eigenvectors[j] = V

        # Propagator: U_j = exp(-iΔt_j H_total)
        Δt = Δts[j]
        propagators[j] = V * Diagonal(cis.(-Δt .* λ)) * V'

        # Forward-propagated state
        states[j+1] = propagators[j] * states[j]
    end

    return KnotPointPropagationData(
        propagators,
        eigenvalues,
        eigenvectors,
        states,
        H_drift,
        H_drives,
        Matrix{Float64}(controls),
        Vector{Float64}(Δts),
    )
end

"""
    ket_vjp(data, λ; use_exact_kick=true, n_quad=4)

Backward-pass vector-Jacobian product (VJP).

Propagates the covector `λ` backward through the knot-point chain, extracting
control gradients at each interval via the sensitivity kick.

Returns a `Matrix{Float64}` of size `[n_drives × (N-1)]` where entry `[k, j]`
is `∂L/∂u_{k,j}` when `λ` is the gradient of the loss w.r.t. the final state.
"""
function ket_vjp(
    data::KnotPointPropagationData,
    λ_seed::AbstractVector;
    use_exact_kick::Bool = true,
    n_quad::Int = 4,
)
    N = length(data.states)
    n_intervals = N - 1
    n_drives = length(data.H_drives)

    grad = zeros(Float64, n_drives, n_intervals)
    λ = ComplexF64.(λ_seed)

    for j = n_intervals:-1:1
        for k = 1:n_drives
            if use_exact_kick
                kick = compute_sensitivity_kick_exact(
                    data.H_drives[k],
                    data.states[j],
                    data.Δts[j],
                    data.eigenvalues[j],
                    data.eigenvectors[j];
                    n_quad = n_quad,
                )
            else
                kick = compute_sensitivity_kick_first_order(
                    data.H_drives[k],
                    data.states[j+1],
                    data.Δts[j],
                )
            end
            grad[k, j] = real(dot(λ, kick))
        end

        # Propagate covector backward: λ ← U_j† · λ
        λ = data.propagators[j]' * λ
    end

    return grad
end

"""
    ket_jvp(data, v; use_exact_kick=true, n_quad=4)

Forward-pass Jacobian-vector product (JVP).

Accumulates the tangent vector `δψ` forward through the knot-point chain:

    δψ_{j+1} = U_j · δψ_j + Σ_k v[k,j] · kick_{k,j}

# Arguments
- `data`: cached propagation data from [`setup_knot_point_propagation`](@ref)
- `v::AbstractMatrix`: tangent directions, size `[n_drives × (N-1)]`

Returns `δψ_N ∈ Vector{ComplexF64}`.
"""
function ket_jvp(
    data::KnotPointPropagationData,
    v::AbstractMatrix;
    use_exact_kick::Bool = true,
    n_quad::Int = 4,
)
    N = length(data.states)
    n_intervals = N - 1
    n_drives = length(data.H_drives)
    n = length(data.states[1])

    δψ = zeros(ComplexF64, n)

    for j = 1:n_intervals
        # δψ_{j+1} = U_j · δψ_j + Σ_k v[k,j] · kick_{k,j}
        δψ = data.propagators[j] * δψ

        for k = 1:n_drives
            if use_exact_kick
                kick = compute_sensitivity_kick_exact(
                    data.H_drives[k],
                    data.states[j],
                    data.Δts[j],
                    data.eigenvalues[j],
                    data.eigenvectors[j];
                    n_quad = n_quad,
                )
            else
                kick = compute_sensitivity_kick_first_order(
                    data.H_drives[k],
                    data.states[j+1],
                    data.Δts[j],
                )
            end
            δψ .+= v[k, j] .* kick
        end
    end

    return δψ
end

# ============================================================================ #
# Tests
# ============================================================================ #

@testitem "Exact kick vs finite differences" begin
    using LinearAlgebra
    using Piccolo
    using Piccolissimo.KnotPointSensitivity:
        compute_sensitivity_kick_exact, compute_sensitivity_kick_first_order

    sys = QuantumSystem(GATES.Z, [GATES.X, GATES.Y], [1.0, 1.0])

    n = sys.levels
    n_drives = sys.n_drives
    H_drift = Matrix{ComplexF64}(sys.H_drift)
    H_drives = [Matrix{ComplexF64}(d.H) for d in sys.H_drives]

    Δt = 0.05
    u = randn(n_drives)
    ψ = normalize(randn(ComplexF64, n))

    # Total Hamiltonian and eigendecomposition
    H_total = copy(H_drift)
    for k = 1:n_drives
        H_total .+= u[k] .* H_drives[k]
    end
    λ, V = eigen(Hermitian(H_total))

    ε = 1e-7
    for k = 1:n_drives
        # Exact kick
        kick = compute_sensitivity_kick_exact(H_drives[k], ψ, Δt, λ, V; n_quad = 8)

        # Finite-difference reference
        u_plus = copy(u)
        u_plus[k] += ε
        H_plus = copy(H_drift)
        for m = 1:n_drives
            H_plus .+= u_plus[m] .* H_drives[m]
        end

        u_minus = copy(u)
        u_minus[k] -= ε
        H_minus = copy(H_drift)
        for m = 1:n_drives
            H_minus .+= u_minus[m] .* H_drives[m]
        end

        Uψ_plus = exp(-im * Δt * H_plus) * ψ
        Uψ_minus = exp(-im * Δt * H_minus) * ψ
        kick_fd = (Uψ_plus - Uψ_minus) / (2ε)

        @test isapprox(kick, kick_fd; atol = 1e-5)
    end
end

@testitem "VJP/JVP duality: λ† · JVP(v) ≈ VJP(λ)ᵀ · v" begin
    using LinearAlgebra
    using Piccolo
    using Piccolissimo: setup_knot_point_propagation, ket_vjp, ket_jvp

    sys = QuantumSystem(GATES.Z, [GATES.X, GATES.Y], [1.0, 1.0])
    n = sys.levels
    n_drives = sys.n_drives

    N = 6
    Δt = 0.1
    controls = 0.5 * randn(n_drives, N)
    Δts = fill(Δt, N - 1)
    ψ0 = normalize(randn(ComplexF64, n))

    data = setup_knot_point_propagation(sys, controls, Δts, ψ0)

    # Random covector and tangent
    λ = normalize(randn(ComplexF64, n))
    v = randn(n_drives, N - 1)

    vjp_grad = ket_vjp(data, λ)
    jvp_out = ket_jvp(data, v)

    # Duality check: Re(λ† · JVP(v)) ≈ VJP(λ)ᵀ · v  (both are real)
    lhs = real(dot(λ, jvp_out))
    rhs = dot(vjp_grad, v)

    @test isapprox(lhs, rhs; atol = 1e-10)
end

@testitem "VJP vs finite differences" begin
    using LinearAlgebra
    using Piccolo
    using Piccolissimo: setup_knot_point_propagation, ket_vjp

    sys = QuantumSystem(GATES.Z, [GATES.X, GATES.Y], [1.0, 1.0])
    n = sys.levels
    n_drives = sys.n_drives

    N = 5
    Δt = 0.1
    controls = 0.3 * randn(n_drives, N)
    Δts = fill(Δt, N - 1)
    ψ0 = normalize(randn(ComplexF64, n))

    data = setup_knot_point_propagation(sys, controls, Δts, ψ0)
    λ = normalize(randn(ComplexF64, n))

    vjp_grad = ket_vjp(data, λ)

    # Finite-difference gradient
    ε = 1e-6
    fd_grad = zeros(n_drives, N - 1)
    for j = 1:(N-1), k = 1:n_drives
        c_plus = copy(controls)
        c_plus[k, j] += ε
        data_plus = setup_knot_point_propagation(sys, c_plus, Δts, ψ0)

        c_minus = copy(controls)
        c_minus[k, j] -= ε
        data_minus = setup_knot_point_propagation(sys, c_minus, Δts, ψ0)

        # f(u) = Re(λ† ψ_N(u))  — the VJP of a linear functional
        fd_grad[k, j] = real(dot(λ, data_plus.states[end] - data_minus.states[end])) / (2ε)
    end

    @test isapprox(vjp_grad, fd_grad; atol = 1e-5)
end

@testitem "JVP vs finite differences" begin
    using LinearAlgebra
    using Piccolo
    using Piccolissimo: setup_knot_point_propagation, ket_jvp

    sys = QuantumSystem(GATES.Z, [GATES.X, GATES.Y], [1.0, 1.0])
    n = sys.levels
    n_drives = sys.n_drives

    N = 5
    Δt = 0.1
    controls = 0.3 * randn(n_drives, N)
    Δts = fill(Δt, N - 1)
    ψ0 = normalize(randn(ComplexF64, n))

    data = setup_knot_point_propagation(sys, controls, Δts, ψ0)
    v = randn(n_drives, N - 1)

    jvp_out = ket_jvp(data, v)

    # Finite-difference JVP
    ε = 1e-6
    c_plus = controls[:, 1:end]  # copy
    c_plus[:, 1:(N-1)] .+= ε .* v
    data_plus = setup_knot_point_propagation(sys, c_plus, Δts, ψ0)

    c_minus = controls[:, 1:end]
    c_minus[:, 1:(N-1)] .-= ε .* v
    data_minus = setup_knot_point_propagation(sys, c_minus, Δts, ψ0)

    jvp_fd = (data_plus.states[end] - data_minus.states[end]) / (2ε)

    @test isapprox(jvp_out, jvp_fd; atol = 1e-5)
end
