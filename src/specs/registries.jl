...
export RegistryEntry, ConstFactory, SYSTEMS, TEMPLATES, INTEGRATORS, WRAPPERS,
    OBJECTIVE_TERMS, SOLVERS, SOLVE_STRATEGIES, PROBLEM_KINDS,
    register_system!, register_template!, register_integrator!, register_wrapper!,
    register_objective_term!, register_solver!, register_strategy!, register_kind!,
    lookup_system, lookup_template, lookup_integrator, lookup_wrapper,
    lookup_objective_term, lookup_solver, lookup_strategy, lookup_kind,
    register_all!
...
for (fn, reg) in ((:register_system!, SYSTEMS), (:register_template!, TEMPLATES),
    (:register_integrator!, INTEGRATORS), (:register_wrapper!, WRAPPERS),
    (:register_objective_term!, OBJECTIVE_TERMS), (:register_solver!, SOLVERS),
    (:register_strategy!, SOLVE_STRATEGIES), (:register_kind!, PROBLEM_KINDS))
    @eval $fn(name::Symbol, e::RegistryEntry) = _register!($reg, name, e)
end
for (fn, reg) in ((:lookup_system, SYSTEMS), (:lookup_template, TEMPLATES),
    (:lookup_integrator, INTEGRATORS), (:lookup_wrapper, WRAPPERS),
    (:lookup_objective_term, OBJECTIVE_TERMS), (:lookup_solver, SOLVERS),
    (:lookup_strategy, SOLVE_STRATEGIES), (:lookup_kind, PROBLEM_KINDS))
...
function register_all!()
    ...
    for (name, fac) in ((:TransmonSystem, TransmonSystem), (:MultiTransmonSystem, MultiTransmonSystem),
        (:TransmonCavitySystem, TransmonCavitySystem), (:RydbergChainSystem, RydbergChainSystem),
        (:IonChainSystem, IonChainSystem), (:RadialMSGateSystem, RadialMSGateSystem),
        (:CatSystem, CatSystem))
        register_system!(name, RegistryEntry(; factory=fac))
    end
    # templates (factory = existing function; compat = pulse/trajectory matrix)
    register_template!(:SmoothPulseProblem, RegistryEntry(; factory=SmoothPulseProblem,
        compat=Dict(:pulse_kinds => [:zero_order], :trajectory_kinds => [:unitary, :ket],
            :ket_free_phase => false)))
    register_template!(:SplinePulseProblem, RegistryEntry(; factory=SplinePulseProblem,
        compat=Dict(:pulse_kinds => [:linear_spline, :cubic_spline],
            :trajectory_kinds => [:unitary, :ket], :ket_free_phase => true)))
    register_template!(:BangBangPulseProblem, RegistryEntry(; factory=BangBangPulseProblem,
        compat=Dict(:pulse_kinds => [:zero_order], :trajectory_kinds => [:unitary, :ket],
            :ket_free_phase => true)))
...
@testitem "registries: register, lookup, idempotency, conflict" begin
    using Piccolo.Specs
    r = Specs.RegistryEntry(; factory=() -> 42, params=Dict(:Q => (; type=:Float64, default=100.0)),
    ...
    @test_throws Exception Specs.register_template!(:MyTmpl,
        Specs.RegistryEntry(; factory=() -> 0))  # conflicting → error
