...
# ## Saving a Pulse
#
# After solving, extract the optimized pulse with `get_pulse` and persist it
# with `save`. The `save(filename, pulse)` method writes the pulse under the
# JLD2 key `"pulse"`, and `load_pulse(filename)` reads it back as the original
# pulse type.
...
# It is also the right loader for a metadata bundle saved with
# `jldsave(...; pulse=...)`:
...
# at every timestep, and timesteps), save the `NamedTrajectory` instead. The
# trajectory-aware `save` writes under the JLD2 key `"traj"` and pairs with
# `load_traj`:
