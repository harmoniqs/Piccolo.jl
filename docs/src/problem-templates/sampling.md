...
```julia
# More weight on nominal system, less on extremes
qcp_robust = SamplingProblem(
    qcp_base,
    [sys_nominal, sys_high, sys_low];
...
```julia
# Sample drift Hamiltonian scaling from 0.9 to 1.1
scales = range(0.9, 1.1, length=11)
systems = [QuantumSystem(s * H_drift, H_drives, drive_bounds) for s in scales]
