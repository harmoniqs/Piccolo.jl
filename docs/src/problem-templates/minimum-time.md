...
```julia
# Try different fidelity targets
for target_fidelity in [0.999, 0.99, 0.95, 0.90]
    qcp_mintime = MinimumTimeProblem(qcp_base; final_fidelity=target_fidelity)
    solve!(qcp_mintime; max_iter=100)

    duration = sum(get_timesteps(get_trajectory(qcp_mintime)))
    actual_fid = fidelity(qcp_mintime)
    println("Target: $target_fidelity, Duration: $duration, Achieved: $actual_fid")
end
```
...
```julia
qcp_mintime = MinimumTimeProblem(qcp_base; goal=GATES[:Y], final_fidelity=0.99)
```
