...
To later use `MinimumTimeProblem`, enable variable timesteps:
...
For multilevel systems, use `PiccoloOptions` to enable leakage handling:

```julia
using Piccolo

# 3-level transmon system
...
Works with `KetTrajectory` for state preparation:
